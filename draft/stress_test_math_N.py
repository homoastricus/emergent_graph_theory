...
# ВЫЧИСЛЕНИЕ p ИЗ ВАРИАЦИОННОГО ПРИНЦИПА
#p = 4.802764507914655e-42
...
def formulas(N):
    ...
    m_k0_meson = (lnN ** 6 * 1 / (4 * (pi ** 2)) * (2 * pi) ** (1 / 2)) / N13
    ...
    Omega0_c = K * lnN ** 6 / (pi ** (5 / 2) * N13)

    lambda_B0 = (lnN ** 6 * pi ** (1 / 2) * N ** (-1 / 3)) / K ** (1 / 2)
    ...
    omega_meson = lnN ** 5 * 2 * pi ** 2 * 2 ** (1 / 2) / N13

    eta_shtrih = lnN**5 * K**3 * 1/(2* pi) /  N**(1/3)

    # ρ-мезон: a=5, b=0, c=1, d=2.5
    # m = √3 · π^(5/2) · (ln N)^5 / N^(1/3)
    m_rho = (math.sqrt(3)) * (pi ** (5/2)) * (lnN ** 5) / N**(1/3)

    # K*-мезон: a=6, b=2, c=0, d=-2.5
    # m = (√2)^2 · π^(-5/2) · (ln N)^6 / N^(1/3)
    # m = 2 · π^(-5/2) · (ln N)^6 / N^(1/3)
    m_K_star = 2 * pi ** (-5/2) * lnN ** 6 / N13

    # B-мезон: a=6, b=-3, c=-3, d=2.0
    # m = (√2)^(-3) · (√3)^(-3) · π^2 · (ln N)^6 / N^(1/3)
    # m = π^2 · (ln N)^6 / (2^(3/2) · 3^(3/2) · N^(1/3))
    m_B = (pi ** 2) * (lnN ** 6) / ((2 ** (3/2)) * (3 ** (3/2)) * N13)

    # η_c-мезон: a=5, b=4, c=6, d=0
    # m = (√2)^4 · (√3)^6 · (ln N)^5 / N^(1/3)
    # m = 4 · 27 · (ln N)^5 / N^(1/3)
    # m = 108 · (ln N)^5 / N^(1/3)
    m_eta_c = (math.sqrt(2))**4 * (math.sqrt(3))**6 * (lnN ** 5) / N13

    # h_c-мезон: a=6, b=1, c=0, d=-1
    # m = √2 · π^(-1) · (ln N)^6 / N^(1/3)
    m_h_c = math.sqrt(2) * (lnN ** 6) / (pi * N13)

    # δ-мезон: a=6, b=-2, c=0, d=-1
    # m = (√2)^(-2) · π^(-1) · (ln N)^6 / N^(1/3)
    # m = (ln N)^6 / (2 · π · N^(1/3))
    m_delta = (lnN ** 6) / (2 * pi * N13)

    # B_c-мезон: a=5, b=6, c=4, d=1
    # m = (√2)^6 · (√3)^4 · π^1 · (ln N)^5 / N^(1/3)
    # m = 8 · 9 · π · (ln N)^5 / N^(1/3)
    # m = 72π · (ln N)^5 / N^(1/3)
    m_B_c = (math.sqrt(2))**6 * (math.sqrt(3))**4 * pi * (lnN ** 5) / N13

    # B_s-мезон: a=6, b=4, c=1, d=0
    # m = (√2)^4 · √3 · (ln N)^6 / N^(1/3)
    # m = 4 · √3 · (ln N)^6 / N^(1/3)
    m_B_s = (math.sqrt(2))**4 * math.sqrt(3) * (lnN ** 6) / N13

    # Ξ^{++}_b-барион: a=6, b=1, c=-2, d=0
    # m = √2 · (√3)^(-2) · (ln N)^6 / N^(1/3)
    # m = √2 · (ln N)^6 / (3 · N^(1/3))
    m_Ksi_pp_b = math.sqrt(2) * (lnN ** 6) / (3 * N13)

    delta_factor = (1-1/(K*lnN))
    return {key: value * delta_factor for key, value in {
        'ħ': hbar_val,
        'h': h_val,
        't_P': tP_val,
        'l_P': lP_val,
...
def main():
    ...
    N0 = math.exp(N_phys)     #4.183e+121 - # 4.197668e121 #4.197668e121
